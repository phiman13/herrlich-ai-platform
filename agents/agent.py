...
def build_system_prompt(memory_context: str) -> str:
    ...
    base = (
        ...
        "delete/mark_read/mark_unread). Für Write-Aktionen zuerst die Mail via "
        "search finden (gibt mail_id zurück), dann mit der mail_id die Aktion "
        "vormerken. Schreib-Aktionen werden vorgemerkt und erst nach Philipps "
        ...
        "- coding: Projekte lesen und Claude Code starten. action='list_projects': "
        "Projekte auflisten. action='query' (project, query_type): Projekt-Info lesen "
        "— query_type: backlog/git_log/readme/claude_md. action='run' (project, task): "
        "Claude Code sofort starten (KEIN Confirm, läuft im Hintergrund). "
